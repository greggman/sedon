...
test('setInputValue stores an override and `name in inputValues` becomes true', () => {
  ...
  useEditorStore.getState().setInputValue(nodeId, 'scale', 12);
  const node = useEditorStore.getState().graph.nodes.find((n) => n.id === nodeId);
  assert.equal(node?.inputValues?.['scale'], 12);
  assert.ok('scale' in (node?.inputValues ?? {}), 'key must be present in inputValues');
});

test('setInputValue(undefined) DELETES the key (not stores undefined)', () => {
  const nodeId = seedWithNode();
  useEditorStore.getState().setInputValue(nodeId, 'scale', 12);
  useEditorStore.getState().setInputValue(nodeId, 'scale', undefined);
  const node = useEditorStore.getState().graph.nodes.find((n) => n.id === nodeId);
  // The dot's display logic uses `!== undefined`, so the key MUST be
  // absent (or be the literal undefined) after a reset. Verify both.
  assert.equal(node?.inputValues?.['scale'], undefined);
  assert.ok(!('scale' in (node?.inputValues ?? {})), 'key must be removed, not just set undefined');
});

test('reset is undoable: original override is restored', () => {
  const nodeId = seedWithNode();
  const store = useEditorStore.getState();
  store.setInputValue(nodeId, 'scale', 12);
  store.setInputValue(nodeId, 'scale', undefined);
  ...
  store.undo();
  const node = useEditorStore.getState().graph.nodes.find((n) => n.id === nodeId);
  assert.ok(!('scale' in (node?.inputValues ?? {})), 'undo must restore to "no override"');
});
