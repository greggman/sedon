// Carve a heightfield texture along a Path. One thread per output
// texel: finds the minimum distance to any segment of the path's
// polyline, then subtracts `depth × falloff(dist)` from the local
// height. Inside `width/2` of the path the full depth is removed; the
// depth smooths out to zero by `width/2 + falloff`. Outside that the
// texel is identical to the input.
//
...
//
// The texture's R channel is treated as world Y in metres (matching
// the rest of the post-refactor heightfield-as-texture model), so
// `depth` is subtracted directly. The output format is substituted at
// shader-compile time so this works for both rgba8unorm (clamped to
// [0,1]) and rgba16float (real altitudes).
...
@group(0) @binding(3) var dst: texture_storage_2d<{{STORAGE_FORMAT}}, write>;
